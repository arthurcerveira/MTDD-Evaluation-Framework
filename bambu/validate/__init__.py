...
def main():
    ...
    argument_parser.add_argument('--randomizations', type=int, help="number of randomizations to run", default=0)
...
def validate(input_train, input_test, model_path, output, randomizations=0):
...
def validate_model(model, X_train, y_train, X_test, y_test, metrics=["accuracy", "recall", "precision", "f1", "roc_auc"], randomizations=100):
    ...
    # [Arthur] Why do we need to copy the model and fit it again?
    ...
    for _ in tqdm.tqdm(range(randomizations)):
        ...
        # [Arthur] Why only shuffle y_train? Shouldn't we shuffle X_train as well?
    ...
    for metric in metrics:
        # [Arthur] The stats metrics only work with >1 randomizations
